...
import { addToDatabase, checkEntryExistence, getDatabase } from "../Helpers/database";
...
export const checkUserExistence = (username: string): User | undefined => {
  return checkEntryExistence(Databases.USERS, (user: User) => user.username === username);
};
...
const createToken = (userId: string): Token => {
  ...
  return addToDatabase(Databases.TOKENS, token, (token: Token) => token.userId !== userId);
};
